...
class SameSampleStatisticsTest(AbstractStatisticsTestCase):
    ...
    def test_size_statistics(self):
        self.assertDictEqual(self._get_value_from_redis('flash', 'size'), {b'14': b'7'})

    def test_uploaded_on_statistics(self):
        self.assertDictEqual(self._get_value_from_redis('flash', 'uploaded_on'), {self.today: b'7'})

    def test_processed_on_statistics(self):
        self.assertDictEqual(self._get_value_from_redis('flash', 'processed_on'), {self.today: b'7'})

    def test_elapsed_time_statistics(self):
        self.assertDictEqual(self._get_value_from_redis('flash', 'elapsed_time'), {b'13': b'7'})

    def test_status_statistics(self):
        self.assertDictEqual(self._get_value_from_redis('flash', 'status'), {b'0': b'7'})
...
class DifferentSamplesStatisticsTest(AbstractStatisticsTestCase):
    ...
    def test_size_statistics(self):
        self.assertDictEqual(self._get_value_from_redis('pe', 'size'), {b'10': b'5', b'20': b'7', b'110': b'3'})

    def test_uploaded_on_statistics(self):
        self.assertDictEqual(self._get_value_from_redis('pe', 'uploaded_on'), {self.today: bytes(str(5 + 7 + 3).encode('utf-8'))})

    def test_processed_on_statistics(self):
        self.assertDictEqual(self._get_value_from_redis('pe', 'processed_on'), {self.today: bytes(str(5 + 7 + 3).encode('utf-8'))})

    def test_elapsed_time_statistics(self):
        self.assertDictEqual(self._get_value_from_redis('pe', 'elapsed_time'), {b'5': bytes(str(7 + 3).encode('utf-8')),
                                                                                b'13': b'5'})

    def test_status_statistics(self):
        self.assertDictEqual(self._get_value_from_redis('pe', 'status'), {b'0': bytes(str(5 + 3).encode('utf-8')),
                                                                          b'2': b'7'})
...
class DeletedResultRevertsSomeStatisticsTest(AbstractStatisticsTestCase):
    ...
    def test_elapsed_time_statistics_reduced_by_one(self):
        self.assertDictEqual(self._get_value_from_redis('flash', 'elapsed_time'), {b'13': b'6'})

    def test_status_statistics_reduced_by_one(self):
        self.assertDictEqual(self._get_value_from_redis('flash', 'status'), {b'0': b'6'})
