...
SERIALIZED_SAMPLE = {'size': 155,
                     ...
                     'sha2': 'c' * 64,
                     'file_type': 'flash',
                     'storage_file_id': '507f1f77bcf86cd799439011',
...
SERIALIZED_JAVA_SAMPLE = {'size': 123,
                          ...
                          'sha2': '3' * 64,
                          'file_type': 'java',
                          'storage_file_id': '507f1f77bcf86cd799439012',
                          'uploaded_on': '2020-01-15',
...
class CreateSampleAPITest(APITestCase):
    ...
    def test_serialize_single_sample(self):
        ...
        response = self.client.post(
            self.create_sample_url, data, format='json')
    ...
    def test_serialize_same_sample_two_times(self):
        ...
        response = self.client.post(
            self.create_sample_url, data, format='json')
        self.assertEqual(response.status_code, 201)
        # it's 1 regardless the sample being already created
        self.assertEqual(response.json()['non_zip_samples'], 1)
        # Only one sample, because the two samples are the same.
        self.assertEqual(Sample.objects.count(), 1)
