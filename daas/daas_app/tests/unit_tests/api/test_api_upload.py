...
class UploadAPITest(NonTransactionalLiveServerTestCase):
    ...
    # Create test that uploads a file, gets the storage file id, then deletes the sample and checks if the file
    # was deleted from storage.
    def test_upload_and_delete(self):
        self.upload_file_through_web_view(CSHARP_SAMPLE)
        self.wait_sample_creation(1)
        sample = Sample.objects.all()[0]
        storage_file_id = sample.storage_file_id
        assert storage_file_id is not None
        assert sample.id > 0
        self.assertIsNotNone(Sample.objects.filter(storage_file_id=storage_file_id).first())
        self.delete_file_through_web_view(sample.id)
        self.assertIsNone(Sample.objects.filter(storage_file_id=storage_file_id).first())
