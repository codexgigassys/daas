...
class SameSampleStatisticsReadTest(AbstractStatisticsTestCase):
    def setUp(self) -> None:
        super().setUp()
        self._create_samples_with_result(file_type='flash', size=14, amount=7, redis_job_status=TaskStatus.DONE.value,
...
class DifferentSamplesStatisticsReadTest(AbstractStatisticsTestCase):
    def setUp(self) -> None:
        super().setUp()
        self._create_samples_with_result(file_type='pe', size=10, amount=5, redis_job_status=TaskStatus.DONE.value,
                                         result_status=ResultStatus.SUCCESS.value, elapsed_time=13)
        self._create_samples_with_result(file_type='pe', size=20, amount=7, redis_job_status=TaskStatus.FAILED.value,
                                         result_status=ResultStatus.FAILED.value, elapsed_time=5)
        self._create_samples_with_result(file_type='pe', size=110, amount=3, redis_job_status=TaskStatus.DONE.value,
